  ...
  let getDrinks = await page.evaluate(() => {
    ...
    items.forEach((item) => {
      ...
      const imagelink = item
        .querySelector(".product-result-image > div > img")
        .getAttribute("src");
      const priceInt = item.querySelector(".price-integer-part").innerText;
      const priceDecimal = item.querySelector(
  ...
  rawDrinks.forEach((rawDrink) => {
    ...
    const drinkInfo = {
      ...
      imageLink: rawDrink.imagelink
        .replace("&fill=solid&fill-color=ffffff", "")
        .replace(/w=\d+&h=\d+/, ""),
      category,
      size,
      store: "kmarket",
...
const getKmarket = async () => {
  ...
  await Promise.all(
    kmarketCategories.map(async (category) => {
