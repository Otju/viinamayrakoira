describe("Drinkpage ", function () {
  ...
  describe("Search tests ", function () {
    ...
    it("store search works (skaupat)", function () {
      cy.get(".dropdown").eq(0).click()
      cy.get(".dropdown-item-text").eq(2).click()
      cy.hasDrink({ sortByField: "pricePerPortion", store: "skaupat" }, 29)
    })
    ...
    it("minMax and sort searches work (and query strings)", function () {
      cy.visit(
        "http://localhost:3000/drinks?&sortByField=size&sortByDescending=true&minprice=2&maxprice=16&maxpercentage=14&minpricePerLitre=3"
      )
      cy.hasDrink(
        {
          sortByField: "size",
          ...
          minMax: [
            { min: 2, max: 16, name: "price" },
            { max: 14, name: "percentage" },
            { min: 3, name: "pricePerLitre" },
          ],
        },
        29
      )
