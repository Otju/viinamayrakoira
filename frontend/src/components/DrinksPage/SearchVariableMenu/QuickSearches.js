...
const QuickSearches = ({ setSearchVariables, searchVariables, emptySearchVariables }) => {
  ...
  searches = searches.map((search) => ({
    ...search,
    selected: Object.entries(search.searches).every(([key, value]) =>
      _.isEqual(searchVariables[key], value)
    ),
  }))
              ...
              <HoverableDropDownText
                ...
                handleClick={() =>
                  setSearchVariables({
                    ...
                    ...(item.selected
                      ? Object.fromEntries(
                          Object.keys(item.searches).map((key) => [key, emptySearchVariables[key]])
                        )
                      : item.searches),
                  })
                }
